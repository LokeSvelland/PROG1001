 ...
 */
void nyAnsatt() {
  if (gAntallAnsatte < MAXANSATTE-1) {   //  Plass til flere ansatte:
     printf("\n\tNy ansatt har nr.%i:\n", ++gAntallAnsatte);
                                         //  Oppretter/lager en ny ansatt:
     gAnsatte[gAntallAnsatte-1] = (struct Ansatt*) malloc(sizeof(struct Ansatt));
                                         //  Ny ansatt leser/setter egne data:
     ansattLesOgSettData(gAnsatte[gAntallAnsatte-1]);
